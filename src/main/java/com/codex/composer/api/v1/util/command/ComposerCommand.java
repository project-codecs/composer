...
public abstract class ComposerCommand implements CommandRegistrationCallback {
    ...
    public Text appendPrefix(Text text) {
        return buildPrefix().copy().append(buildPrefix().getString().isBlank() ? Text.empty() : ScreenTexts.space()).append(text);
    }
     ...
     * <p>
     * Must be implemented by subclasses that wish to use a prefix.
     * </p>
     *
     * @return the prefix text
     */
    protected Text buildPrefix() { return Text.empty(); }
    ...
    protected final boolean shouldCancel() {
        return debugOnly() && !ComposerServerConfig.INSTANCE.developerMode.get();
    }
